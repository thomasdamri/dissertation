...
RSpec.describe Criteria, type: :model do
  ...
  it 'is valid with valid attributes' do
    # Get first assessment, created earlier to attach the criteria to
    a = Assessment.first
    # Create a non-assessed string type criteria
    c1 = build(:criteria, assessment: a)
    c2 = build(:weighted_criteria, assessment: a)
  ...
  it 'is invalid with a title that is too long' do
    ...
    # Create a non-assessed string type criteria
    c1 = build(:criteria, assessment: a)
    c1.title = "a" * Criteria.max_title_length
    expect(c1).to be_valid

    c1.title = "a" * (Criteria.max_title_length + 1)
    expect(c1).to_not be_valid
  end

  it 'is invalid when max value is less than the min value' do
    c1 = build(:weighted_criteria, assessment: Assessment.first, min_value: 11)
    expect(c1).to_not be_valid
    c1.min_value = 1
    expect(c1).to be_valid
  end
  ...
  it 'is invalid when assessed, with no min or max value' do
    c1 = build(:weighted_criteria, assessment: Assessment.first, min_value: nil, max_value: nil)
    expect(c1).to_not be_valid
    c1.min_value = 1
    c1.max_value = 10
    expect(c1).to be_valid
  ...
  it 'is invalid when the weighting is 0 or lower' do
    c1 = build(:weighted_criteria, assessment: Assessment.first, weighting: 0)
    expect(c1).to_not be_valid
    c1.weighting = -1
    expect(c1).to_not be_valid
    c1.weighting = 1
  ...
  it 'is invalid when a boolean criteria has a missing label' do
    c1 = build(:criteria, assessment: Assessment.first, min_value: nil, max_value: nil, response_type: Criteria.bool_type)
    expect(c1).to_not be_valid
    c1.min_value = "Yes"
    expect(c1).to_not be_valid
    c1.max_value = "No"
  ...
  it 'is invalid if the response_type is outside the allowed range' do
    c1 = build(:criteria, assessment: Assessment.first, response_type: 550)
    expect(c1).to_not be_valid
    c1.response_type = Criteria.string_type
    expect(c1).to be_valid
  end

  describe "#type_dict" do
    specify "It returns the correct value" do
      expect(Criteria.type_dict["Text"]).to eq 0
    end
  end

  describe "#subtitle" do
    specify "it returns nil for string or bool responses" do
      c1 = build :criteria, response_type: Criteria.string_type
      c2 = build :criteria, response_type: Criteria.bool_type
    ...
    specify "It returns the min, max, or both values for integer and float values" do
      c1 = build :criteria, response_type: Criteria.int_type
      expect(c1.subtitle).to eq "Enter a whole number"

      c1.response_type = Criteria.float_type
